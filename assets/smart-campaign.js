...
async function selectAvatarImage(image) {
    ...
    if (image.width && image.height && image.width !== image.height) {
        showSmartToast(`Creating square version of ${image.width}x${image.height} image...`, 'info');
        console.log('🔄 Non-square image detected, creating cropped version...');
        
        try {
            // Auto-crop and upload the image
            const cropResponse = await smartApiRequest('auto_crop_and_upload', {
            ...
            if (cropResponse.success) {
                ...
                showSmartToast(`✅ Square version (${dimensions}) created and uploaded to TikTok!`, 'success');
            ...
            showSmartToast('Failed to create square version. You can still use the original image.', 'warning');
